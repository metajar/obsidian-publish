...
- API contract frozen between plugin and server: bare-array `GET /api/pages`, `DELETE`→204, PUT password omitted=keep / null=remove / string=set (a password-only PUT body — no markdown/title — is proven by `TestUpdatePagePasswordOnly` in `server/internal/httpapi` to leave content, title, and theme override intact), 401/409/400/404 error mapping.
