...
last_updated: 2026-09-11 (plugin: unpublish note command)
---
...
- Obsidian plugin (`plugin/`): publish command (palette + editor context menu), unpublish command (`unpublish-current-note`, palette + editor context menu, gated on a local route record; verifies route availability against the server before confirming — free route drops the stale mapping without deleting; DELETE 404 also cleans up, other failures keep the mapping), ribbon icon (`send`) and settings "Publish current note" button opening the publish modal for the active note, publish modal (slug prefill, debounced availability check, CSPRNG password generation surfaced via the shared show-once `ShowPasswordModal`, update-mode password semantics), settings tab (server URL + masked token, test connection, live published-pages table with confirmed unpublish + copy link + per-row key-round "generate/replace password" action that does a password-only PUT and shows the new password once — plaintext never stored or logged; theme section with 4 built-in presets + custom CSS pushed via POST /api/theme, loaded live via GET /api/theme), image publishing on publish/re-publish (wiki `![[img.png|400]]` and local `![alt](path.png)` embeds uploaded via POST /api/assets and rewritten to server URLs in the outbound payload only — the note on disk is never modified; remote images, non-image attachments reported via Notice, upload failures fail soft, dedupe per vault file). `npm run build` clean (zero TS errors), 107/107 vitest tests pass.
...
- API contract frozen between plugin and server: bare-array `GET /api/pages`, `DELETE`→200 + `{"deleted": route}` (changed from 204 in 0.0.4-era server: Obsidian's `requestUrl` throws "Unexpected end of JSON input" on empty response bodies, which the plugin misreported as an unreachable server), PUT password omitted=keep / null=remove / string=set (a password-only PUT body — no markdown/title — is proven by `TestUpdatePagePasswordOnly` in `server/internal/httpapi` to leave content, title, and theme override intact), 401/409/400/404 error mapping. No other endpoint returns an empty-body 2xx.
