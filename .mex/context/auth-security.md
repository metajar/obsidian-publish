...
- On successful password entry, the server issues a **short-lived session scoped to that route** (cookie vs. signed URL token — [TO BE DETERMINED — populate after first implementation; see decisions.md]). It is not a permanent unlock.
...
[TO BE DETERMINED — populate after first implementation: Echo middleware wiring for bearer auth and rate limiting, argon2id parameter choices, session issuance/verification code paths.]
